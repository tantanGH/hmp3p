...
PhantomXのバージョンは人柱版を使ってください。ただし、当方の環境では最新の1.20を使った場合「エラーが発生しました」の白窓がたまに出ます。1.04b では安定しています。

特殊文字(スペース、マルチピリオド、シングルクオーテーション等)を含むファイル名には対応していません。
Human68kの標準状態で扱うことのできる18+3文字のファイル名にリネームしておいてください。
