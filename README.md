...
必ずコピーバックモードを使用してください。060turbo.sysに`-cm1`オプションを指定する必要があります。

MP3データファイルは直接ハイメモリに読み込むため、060turbo.sysに`-ss`オプションを指定する必要があります。

Mercury-UNITを使用する場合、現在の実装では44.1kHzフルレートでの再生はかなりギリギリです。もしバッファアンダーランが発生するようであれば、`-b8`など、バッファサイズを少し多めにとってみてください。

CONFIG.SYSにPROCESS=行の記述があると、Timer-D割り込みが使用されるため再生が間に合わなくなる場合があります。

060専用バイナリ `HMP3P060.X` を使用してください。

060専用バイナリは060turbo実機での安定再生のために最適化されています。000版/030版よりも初期バッファを多くとる設定になっています。
...
必ずライトバックモードを使用してください。

68030モードを使っている場合は 030専用バイナリ `HMP3P030.X` の利用を推奨します。

SCSIディスクから読み込む場合は TS16FILE.X を使用する必要があります。できるだけ WindrvXM/VDISK を使用してください。

逐次ディスク読み込みを積極的に行うため再生開始時間までの読み込み時間が短い、000版または030版バイナリの利用を推奨します。
...
XEiJでPCM8PP.Xを使用する場合、常駐前に MKMU.R または MU4ON.X を実行する必要があります。
...
ベースラインJPEGとプログレッシブJPEGに対応しています。PNGには対応していません。

`-q1` でハーフレート再生となり処理負担が減りますが、Mercury-UNIT V3.5以上が必須となります。

WindrvXM/VDISKドライブ以外のSCSIドライブ等からMP3を読み込む場合は TS16FILE.X を活用してください。060turboの場合は060turbo.sysに`-ss`オプションを指定してください。
...
* 0.9.8 (2026/06/08) ... 030バイナリは68030専用とした。初期バッファリング中にSHIFTで抜けるとメモリリークしていたのを修正
* 0.9.7 (2026/05/31) ... ハイメモリドライバの存在チェック修正
* 0.9.6 (2026/05/04) ... アルバムアートワークの表示高速化。プログレッシブJPEG対応。逐次ディスク読み込み対応。一部コマンドラインオプション廃止。
* 0.9.4 (2026/04/25) ... バッファの切れ目でブツノイズが入ることがあったのを修正。ID3 v2.2タグ表示に対応した。
