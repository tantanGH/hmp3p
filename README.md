...
必ずコピーバックモードを使用してください。060turbo.sysに`-cm1`オプションを指定してください。

MP3データファイルは直接ハイメモリに読み込むため、TS16FILE.Xの利用を推奨します。あるいは `-s`オプションをつけて読み込みを行ってください。
...
CONFIG.SYSのPROCESS=行の記述があると、Timer-D割り込みが使用されるため再生が間に合わなくなる場合があります。
